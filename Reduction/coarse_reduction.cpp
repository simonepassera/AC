/* Solution of parallel reduction on GPU with the STRIDED+SMEM approach with Thread Coarsening (segmented version) */
...
__global__ void coarse_reduction(int *A, int L, int *sum, int K)
{
    extern __shared__ int smem[];
    unsigned int segment = K * 2 * blockDim.x * blockIdx.x;
...
{
    if (argc != 4) {
        std::cerr << "Usage: " << argv[0] << " <L> <block_size (power of two)> <coarse_factor (power of two)>" << std::endl;
        exit(1);
    }

    int L = atoi(argv[1]);
    int block_size = atoi(argv[2]);
    int coarse_factor = atoi(argv[3]);
    ...
    if (!isPowerOfTwo(coarse_factor)) {
        std::cerr << "coarse_factor is not a power of two!" << std::endl;
        exit(1);
    }
    ...
    *host_result = 0;
    ...
    unsigned int numBlocks = std::ceil(((double) L) / (2*block_size*coarse_factor));

    coarse_reduction<<<numBlocks, block_size, block_size * sizeof(int)>>>(dev_A, L, dev_result, coarse_factor);
