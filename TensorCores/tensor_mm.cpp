/* Matrix multiplication in CUDA with tiling and tesor cores */
...
#define TILE_DIM 16 // do not change this
...
{
    __shared__ half A_smem[TILE_DIM][TILE_DIM];
    __shared__ half B_smem[TILE_DIM][TILE_DIM];
    __shared__ float C_smem[TILE_DIM][TILE_DIM];

    int ix = (blockIdx.x * blockDim.x) + threadIdx.x;
    int iy = (blockIdx.y * blockDim.y) + threadIdx.y;

    C_smem[threadIdx.y][threadIdx.x] = 0.0f;

    for (int t=0; t<(std::ceil(((float) M)/TILE_DIM)); t++) {
        if (iy < N && (t * TILE_DIM) + threadIdx.x < M) {
            A_smem[threadIdx.y][threadIdx.x] = __float2half(A[(iy * M) + (t * TILE_DIM) + threadIdx.x]);
        }
        else {
            A_smem[threadIdx.y][threadIdx.x] = 0.0f;
        }
        if (ix < R && (t * TILE_DIM) + threadIdx.y < M) {
            B_smem[threadIdx.y][threadIdx.x] = __float2half(B[(t * TILE_DIM + threadIdx.y) * R + ix]);
        }
        else {
        ...
        // only the first warp of the block does the calculation
        if ((blockDim.x * threadIdx.y) + threadIdx.x < 32) {
            ...
            // load fragments
            wmma::load_matrix_sync(A_frag, &A_smem[0][0], 16);
            wmma::load_matrix_sync(B_frag, &B_smem[0][0], 16);
            wmma::load_matrix_sync(C_frag, &C_smem[0][0], 16, wmma::mem_row_major);

            // perform the MMA
            wmma::mma_sync(D_frag, A_frag, B_frag, C_frag);

            // store the output
            wmma::store_matrix_sync(&C_smem[0][0], D_frag, 16, wmma::mem_row_major);
        }
        __syncthreads();
    ...
    if (ix < R && iy < N) {
        ...
        C[iy*R + ix] = C_smem[threadIdx.y][threadIdx.x];
    }
}
...
int main(int argc, char **argv)
{
    ...
    if ((N % TILE_DIM != 0)|| (M % TILE_DIM != 0) || (R % TILE_DIM != 0)) {
        std::cout << "N, M and R are not multiple of " << TILE_DIM << std::endl;
        abort();
    }
